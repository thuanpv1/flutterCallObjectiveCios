...
- (void)layoutSubviews{
    ...
    if (@available(iOS 11.0, *)) {
        _selfHeightConstraint.constant = _size.height + self.superview.safeAreaInsets.bottom; // Causes layoutSubviews to be called again
...
-(UIButton*)addBarButton:(NSString*)name normalImage:(nullable NSString*)normal selectedImage:(nullable NSString*)selected disabledImage:(nullable NSString*)disabled autoSelect:(BOOL)autoSelect atPosition:(BottomBarPosition)position{
    ...
    if(normal) [button setImage:[UIImage imageNamed:normal] forState:UIControlStateNormal];
